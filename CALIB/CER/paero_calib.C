...
//
// Calibrate Cherenkov detector by localizing SPE peaks.
...
void paero_calib(string rootname = "") {
  ...
  tree->SetBranchAddress("K.cer.goodPosAdcPulseInt",adc_pos,&b_adc_pos);
  tree->SetBranchAddress("K.cer.goodNegAdcPulseInt",adc_neg,&b_adc_neg);

  TH1D* hpos[NPMT];
  TH1D* hneg[NPMT];

  for (int i=0; i<NPMT; i++) {
    hpos[i] = new TH1D(Form("hpos%d",i+1), Form("ADC+ %i",i+1), NBin, MinAdc, MaxAdc);
    hneg[i] = new TH1D(Form("hneg%d",i+1), Form("ADC- %i",i+1), NBin, MinAdc, MaxAdc);
  }
  ...
  double flo_neg[NPMT] {-2.};
  double fhi_neg[NPMT] {5.};

  double flo_pos[NPMT] {-2.};
  double fhi_pos[NPMT] {5.};
  */
